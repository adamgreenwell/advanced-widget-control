...
if( !function_exists( 'widgetcontrol_tab_settings' ) ):
   function widgetcontrol_tab_settings( $args ){ ?>
       <li class="extended-widget-opts-tab-class">
          <a href="#extended-widget-opts-tab-<?php echo $args['id'];?>-class" title="<?php _e( 'Class, ID & Logic', 'advanced-widget-control' );?>" ><span class="dashicons dashicons-admin-generic"></span> <span class="tabtitle"><?php _e( 'Other Settings', 'advanced-widget-control' );?></span></a>
      </li>
   <?php
   }
   add_action( 'extended_widget_opts_tabs', 'widgetcontrol_tab_settings' );
endif;
...
if( !function_exists( 'widgetcontrol_tabcontent_settings' ) ):
               ...
               <ul class="extended-widget-opts-settings-tabnav-ul">
                   <?php if( 'activate' == $widget_options['hide_title'] ){ ?>
                       <li class="extended-widget-opts-settings-tab-title">
                           <a href="#extended-widget-opts-settings-tab-<?php echo $args['id'];?>-title" title="<?php _e( 'Misc', 'advanced-widget-control' );?>" ><?php _e( 'Misc', 'advanced-widget-control' );?></a>
                       </li>
                   <?php } ?>

                   <?php if( 'activate' == $widget_options['classes'] ){ ?>
                       <li class="extended-widget-opts-settings-tab-class">
                           <a href="#extended-widget-opts-settings-tab-<?php echo $args['id'];?>-class" title="<?php _e( 'Class & ID', 'advanced-widget-control' );?>" ><?php _e( 'Class & ID', 'advanced-widget-control' );?></a>
                       </li>
                   <?php } ?>

                   <?php if( 'activate' == $widget_options['logic'] ){ ?>
                       <li class="extended-widget-opts-settings-tab-logic">
                           <a href="#extended-widget-opts-settings-tab-<?php echo $args['id'];?>-logic" title="<?php _e( 'Display Logic', 'advanced-widget-control' );?>" ><?php _e( 'Logic', 'advanced-widget-control' );?></a>
                       </li>
                   <?php } ?>
                       ...
                       <div class="widget-opts-logic">
                           ...
                           <?php if( !isset( $widget_options['settings']['logic'] ) ||
                                    ( isset( $widget_options['settings']['logic']  ) && !isset( $widget_options['settings']['logic']['notice']  ) ) ){ ?>
                                         <p><a href="#" class="widget-opts-toggler-note"><?php _e( 'Click to Toggle Note', 'advanced-widget-control' );?></a></p>
                                         <p class="widget-opts-toggle-note"><small><?php _e( 'PLEASE NOTE that the display logic you introduce is EVAL\'d directly. Anyone who has access to edit widget appearance will have the right to add any code, including malicious and possibly destructive functions. There is an optional filter <em>"widget_options_logic_override"</em> which you can use to bypass the EVAL with your own code if needed.', 'advanced-widget-control' );?></small></p>
                           <?php } ?>
                       </div>
   ...
   add_action( 'extended_widget_opts_tabcontent', 'widgetcontrol_tabcontent_settings');
endif; ?>
