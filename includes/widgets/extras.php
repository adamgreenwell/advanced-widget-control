...
if( !function_exists( 'widgetcontrol_classes_generator' ) ){
    function widgetcontrol_classes_generator( $opts, $tabs, $settings, $so = false ){
        if( !empty( $opts ) && is_array( $opts ) ){
            ...
            if( $so && 'activate' == $tabs['hide_title'] ){
                ...
                if( isset( $custom_class['title'] ) && !empty( $custom_class['title'] ) ){
                    $classes[] = 'widgetcontrol-hide_title';
                }
            }

            return apply_filters( 'widgetcontrol_get_classes', $classes );
        }
    }
}

//add is_active_sidebar support
if( !function_exists( 'widgetcontrol_sidebars_widgets' ) ){
	add_action( 'wp_loaded', 'widgetcontrol_sidebars_widgets_action' );
	function widgetcontrol_sidebars_widgets_action() {
        if( apply_filters( 'widgetcontrol_is_active_sidebar_support', true ) ){
    		add_filter( 'sidebars_widgets', 'widgetcontrol_sidebars_widgets' );
        }
	}
	function widgetcontrol_sidebars_widgets( $sidebars ) {
		if ( is_admin() ) {
			return $sidebars;
		}
		...
		foreach ( $sidebars as $s => $sidebar ) {
			...
			foreach ( $sidebar as $w => $widget ) {
				...
				if ( isset( $checked[ $widget ] ) ) {
					...
					$show = widgetcontrol_display_callback( $instance, (object) array( 'id' => $widget ), '' );
